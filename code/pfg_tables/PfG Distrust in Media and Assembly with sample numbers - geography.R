...
for (question in questions) {
     ...
     if ("LGD2014name" %in% names(pfg_data_year)) {
          ...
          for (LGD in LGDs) {
            ...
            p_weighted <- pfg_data_year %>%
              filter(!is.na(.[[question]]) & .[["LGD2014name"]] == LGD & .[[question]] %in% c("Tend to distrust/Distrust greatly", "Tend to distrust/distrust greatly")) %>%
              pull(weight) %>%
              sum() / pfg_data_year %>%
  ...
  if ("AsmblyArea" %in% names(pfg_data_year)) {
    ...
    for (AAname in AAs) {
      ##### Weighted p ####
      p_weighted <- pfg_data_year %>%
        filter(!is.na(.[[question]]) & .[["AsmblyArea"]] == AAname & .[[question]] %in% c("Tend to distrust/Distrust greatly", "Tend to distrust/distrust greatly")) %>%
        pull(weight) %>%
        sum() / pfg_data_year %>%
  ...
  addWorksheet(wb_LGD, paste0("Dis",question), tabColour = "#00205B")

  writeDataTable(wb_LGD, paste0("Dis",question),
    x = question_lgd_data_rounded,
    tableStyle = "none",
    withFilter = FALSE
  )

  addStyle(wb_LGD, paste0("Dis",question),
    style = ns_pfg,
  ...
  setColWidths(wb_LGD, paste0("Dis",question),
    cols = 1:7,
...
addWorksheet(wb_AA, paste0("Dis",question), tabColour = "#00205B")

writeDataTable(wb_AA, paste0("Dis",question),
               x = question_aa_data_rounded,
               tableStyle = "none",
               withFilter = FALSE
)

addStyle(wb_AA, paste0("Dis",question),
         style = ns_pfg,
...
setColWidths(wb_AA, paste0("Dis",question),
             cols = 1:7,
             widths = c(22.86, 14.14, 12.86, 52.43, 10.14, 10.14, 10.71)
)
...
xl_lgd_filename <- paste0(here(), "/outputs/PfG/PfG - Distrust LGD Data (pfg covariates) with sample numbers ", current_year, ".xlsx")
xl_aa_filename <- paste0(here(), "/outputs/PfG/PfG - Distrust Assembly Area Data (pfg covariates) with sample numbers ", current_year, ".xlsx")
