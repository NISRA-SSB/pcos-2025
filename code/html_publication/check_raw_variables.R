...
for (i in 1:length(PCOS_vars)) {
  ...
  frequency_table <- frequency_table %>%
    mutate(
      unweighted_last = case_when(
        is.na(unweighted_last) ~ 0,
        TRUE ~ unweighted_last
      ),
      weighted_last = case_when(
        is.na(weighted_last) ~ 0,
        TRUE ~ weighted_last
      ),
      unweighted_current = case_when(
        is.na(unweighted_current) ~ 0,
        TRUE ~ unweighted_current
      ),
      weighted_current = case_when(
        is.na(weighted_current) ~ 0,
    ...
    select(
      var, weighted_last_pct, weighted_current_pct, unweighted_current_pct, unweighted_last, unweighted_current, unweighted_last_pct,
