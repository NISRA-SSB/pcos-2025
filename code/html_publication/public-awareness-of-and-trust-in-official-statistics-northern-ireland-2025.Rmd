...
-   In `r current_year`, almost half of respondents (`r heard_of_nisra`%) indicated that they had heard of the Agency before being contacted about the survey. This is higher than 2024, when 42% of respondents had heard of NISRA.


**Trust**

-   Public trust in NISRA, as an institution, is high (`r trust_in_nisra`%). Of those that had heard of NISRA, 92% stated that they trusted the Agency.
...
-   Four-fifths (`r free_from_interference`%) of respondents agree that statistics produced by NISRA are free from political interference.
...
In the UK, the Office for National Statistics (ONS) is the largest producer of official statistics. The most recent ONS survey, monitoring public trust in the ONS and the statistics it produces, was conducted in `r ons_year` ([Public Confidence in Official Statistics `r ons_year` (ONS)](https://natcen.ac.uk/publications/public-confidence-official-statistics)). The ONS survey samples respondents in England, Scotland and Wales whilst the NISRA survey samples respondents in Northern Ireland. This report contains comparisons between NISRA and ONS.
...
Awareness is higher than in 2024, when 42% of respondents had heard of NISRA, but similar to 2022 (49%) and 2023 (48%). Public awareness of NISRA has decreased since 2020 and 2021 (during the COVID-19 pandemic) but remains notably higher than in 2019 and earlier years (Chart 1).
...
Public awareness of NISRA remains lower than awareness of ONS, with nearly three quarters (`r aware_of_ons`%) of respondents having heard of ONS in 2023 (the latest available data). The difference between public awareness levels for NISRA and ONS has narrowed since 2018, which was before the COVID-19 pandemic (Chart 2).
...
Respondents were asked whether they had heard of specific NISRA statistics. 33% of had not heard of any of the statistics listed, while 10% had heard of all the statistics. A more detailed breakdown is available in the accompanying tables (see [Appendix B](public-awareness-of-and-trust-in-official-statistics-northern-ireland-`r current_year`-appendix-b.html)).

Just over half of respondents were aware of statistics on the number of people who live in Northern Ireland (`r aware_people`%), statistics on hospital waiting times (`r aware_hospital`%) and the unemployment rate (`r aware_unemployment`%). However, less than one in five (`r aware_cycling`%) of respondents had heard of statistics on journeys made by walking, cycling or public transport in Northern Ireland (Chart 3).
...
There is no significant difference in the level of trust in NISRA in 2025 compared with 2024 (79%) and 2023 (83%). Since 2020 (during the COVID-19 pandemic), trust in NISRA has decreased from 90% and the percentage of respondents answering 'don't know' has increased from 7% to `r dont_know_trust_nisra`%. Current trust levels are comparable to those recorded in 2019 and 2016 (Chart 5).

When only respondents who expressed an opinion are considered, i.e. excluding ‘don’t knows’, trust in NISRA is higher than in 2024, similar to levels recorded between 2020 and 2023 and higher than in 2019 and earlier years.
...
Trust in NISRA in 2025 is not significantly different to trust in ONS (`r trust_in_ons`%) in 2023, the latest available data (Chart 6). However, when only those who gave an opinion are considered, i.e. excluding 'don't knows', trust in NISRA is higher.
...
Trust in NISRA statistics in 2025 is similar to 2024, when 83% of respondents expressed trust. Since 2020 (during the COVID-19 pandemic), trust in NISRA statistics has decreased from 90% to 82%, and the percentage of respondents answering ‘don’t know’ has increased from 6% to 15%. The level of trust in 2025 is similar to 2019 and earlier years.

When only those respondents who expressed an opinion are considered, i.e. excluding ‘don’t knows’, trust in NISRA statistics in 2025 is higher than 2024, similar to that recorded in 2020 to 2023 and higher than 2019 and earlier years.
...
### Reasons for trusting NISRA statistics, `r current_year`

Respondents who indicated that they trust NISRA statistics were asked their reasons for doing so and could select up to three reasons from a predetermined list (Chart 10). The most frequently chosen reasons were that official statistics in Northern Ireland are generally accurate (`r Reasons_trust_accurate`%), that they trust the statistics from a personal experience (`r Reasons_trust_experience`%) and that NISRA does not have a vested interest in or manipulate the figures (`r Reasons_trust_nisra_vested`%).  

#### Chart 10: Reasons for trusting NISRA statistics, `r current_year`

```{r Chart 10, fig.alt = "Vertical bar chart, Reasons for trusting NISRA.", fig.height=6}

wrap_label <- function(x, width = 16) {
  sapply(x, function(s) paste(str_wrap(s, width = width), collapse = "<br>"))
}

percent_weighted_df$option_wrapped <- wrap_label(percent_weighted_df$option_label, width = 30)

plot_ly(
  data = percent_weighted_df,
  x = ~percent_weighted,
  y = ~option_wrapped,
  type = "bar",
  orientation = "h",
  marker = list(color = nisra_navy),
  text = ~paste0(round_half_up(percent_weighted)), # add text inside bars
  textposition = "inside",
  insidetextanchor = "end",
  texttemplate = "<b>%{x:.0f}</b>",
  cliponaxis = FALSE,
  hovertext = ~paste0(option_label, ": ", round_half_up(percent_weighted), "%"),
  hoverinfo = "text"
) %>%
  layout(font = list(family = "Arial", size = 12),
         bargap = 0.45,
         ...
         xaxis = list(title = "Percentage",
                      ...
                      fixedrange = TRUE),
         yaxis = list(categoryorder = "array",  # How to order the categories
                      categoryarray = rev(percent_weighted_df$option_wrapped),
                      title = "",
                      fixedrange = TRUE,
                      ticks = "outside",
                      ticklen = 2,
...
f_extract_sheet(path = xl_filename,
  ...
  sheet_title = paste0("Table 4.1a: Reasons for trusting NISRA statistics, ", current_year))

```

### Reasons for distrusting NISRA statistics, `r current_year`

Similarly, respondents who indicated they do not trust NISRA statistics were asked their reasons for not doing so (Chart 11). The reason most frequently chosen by respondents was that the statistics alone do not tell the whole story (`r Reasons_distrust_story`%). The next most common reason given was that Government departments have a vested interest in or manipulate the figures (`r Reasons_distrust_gov_vested`%).  

#### Chart 11: Reasons for distrusting NISRA statistics, `r current_year`

```{r Chart 11, fig.alt = "Vertical bar chart, Reasons for distrusting NISRA.", fig.height=7}

wrap_label <- function(x, width = 16) {
  sapply(x, function(s) paste(str_wrap(s, width = width), collapse = "<br>"))
}

percent_weighted_distrust_df$option_wrapped <- wrap_label(percent_weighted_distrust_df$option_label, width = 30)

plot_ly(
  data = percent_weighted_distrust_df,
  x = ~percent_weighted,
  y = ~option_wrapped,
  type = "bar",
  orientation = "h",
  ...
  layout(font = list(family = "Arial", size = 12),
         ...
         xaxis = list(title = "Percentage",
                      ...
                      fixedrange = TRUE),
         yaxis = list(categoryorder = "array",  # How to order the categories
                      categoryarray = rev(percent_weighted_distrust_df$option_wrapped),
                      title = "",
                      fixedrange = TRUE,
                      automargin = TRUE,
                      ticks = "outside",
                      ticklen = 2,          # very small
                      tickwidth = 0,        # invisible
                      tickcolor = "rgba(0,0,0,0)",
                      tickpadding = 8),
         margin = list(l = 0, r = 0)) %>%
  config(displayModeBar = FALSE)
...
f_extract_sheet(path = xl_filename,
  ...
  sheet_title = paste0("Table 4.2a: Reasons for distrusting NISRA statistics, ", current_year))
...
The percentage of respondents agreeing that NISRA statistics are important to understand Northern Ireland in 2025 is not significantly different from 2022 to 2024 levels, lower than in 2020 and 2021 and comparable to 2019 and 2016 (Chart 12).

When only those respondents who had a view are considered, i.e. excluding 'don't knows', agreement that NISRA statistics are important to understand Northern Ireland is higher than in 2024, similar to 2020 to 2023 and higher than 2019 and 2016.


#### Chart 12: NISRA statistics are important to understand Northern Ireland (2016-`r current_year`)

```{r Chart 12, fig.alt = "Vertical bar chart showing belief that NISRA statistics are important to understand Northern Ireland - agree responses in dark blue, disagree responses in light blue, and don't know responses in grey. Each bar represents a different year."}
...
The percentage of NISRA respondents (`r importance_of_stats`%) who agree that the statistics produced are important to understand their country is similar to ONS respondents (`r importance_of_ons`%).

While NISRA respondents are more likely to answer ‘don’t know’ (`r dont_know_importance`%) than ONS respondents (`r dont_know_importance_ons`%), the percentage of respondents who disagree is notably lower for NISRA (`r disagree_importance`%) than it is for ONS (`r disagre_importance_ons`%) (Chart 13).


#### Chart 13: Statistics produced are important to understand our country, NISRA (`r current_year`) and ONS (`r ons_year`)

```{r Chart 13, fig.alt = "Vertical bar chart showing belief that statistics produced by each institution are important in understanding the country. 89% for NISRA in 2022, 87% for ONS in 2021."}
...
Four-fifths (`r free_from_interference`%) of respondents agree that statistics produced by NISRA are free from political interference. Less than one in twenty (`r disagree_interference`%) respondents disagree that NISRA statistics are free from political interference whilst the remaining respondents (`r dont_know_interference`%) answered that they ‘don’t know’ (Chart 14).

### Comparison over time

Agreement that NISRA statistics are free from political interference is not significantly different to earlier years (2021 to 2024) but is lower than in 2020 (88%). However, when only those respondents who expressed an opinion are considered, agreement has increased since 2023 and 2024 and is similar to 2020 to 2022 levels. 

The percentage of respondents disagreeing that NISRA statistics are free from political interference (`r disagree_interference`%) has decreased since 2024 (10%).

#### Chart 14: NISRA statistics are free from political interference (2014-`r current_year`)
...
In comparison, `r free_from_interference_ons`% of respondents agree that statistics produced by ONS are free from political interference (Chart 15). This is significantly lower than the NISRA figure.

#### Chart 15: Statistics produced are free from political interference, NISRA (`r current_year`) and ONS (`r ons_year`)

```{r Chart 15, fig.alt = "Vertical stacked bar chart showing belief that statistics are free from political interference. Belief that NISRA statistics are free from interference is at 80% in 2023 (dark blue segment), compared to 67% for ONS in 2023."}
...
Ninety-one percent of respondents (`r agree_confidential`%) agree that personal information provided to NISRA will be kept confidential while `r disagree_confidential`% disagree. The remaining respondents 'don’t know' (`r dont_know_confidential`%) (Chart 16).

### Comparison over time

The percentage of respondents agreeing that personal information provided to NISRA will be kept confidential is higher than in 2023 and 2024, but similar to 2021 and 2022 and lower than in 2020. 

Disagreement with the statement is lower than in 2023 and 2024, but similar to earlier years (2020 to 2022). 

#### Chart 16: Personal information provided to NISRA will be kept confidential (2014-`r current_year`)

```{r Chart 16, fig.alt = "Vertical bar chart, each bar represents a different year. Agree responses are in dark blue, disagree responses in light blue, don't know responses are in grey."}
...
In comparison, `r agree_confidential_ons`% of ONS respondents in `r ons_year` (the latest available data) agree that personal information provided to ONS will be kept confidential. This is significantly lower than for NISRA (Chart 17).

#### Chart 17: Belief that personal information provided will be kept confidential, NISRA (`r current_year`) and ONS (`r ons_year`)

```{r Chart 17, fig.alt = "Vertical bar chart showing belief that personal information will be kept confidential by institution. In 2023, 85% believed that NISRA would keep personal information confidential (dark blue segment), compared to 88% for ONS in 2023."}
